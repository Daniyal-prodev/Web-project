  ...
  return (
    <div className="max-w-md mx-auto px-4 py-12">
      ...
      <form onSubmit={onSubmit} className="space-y-3 bg-white rounded-xl shadow p-6">
        ...
        <button disabled={loading} className="w-full bg-pink-600 text-white rounded py-2">{loading ? "Creating..." : "Sign up"}</button>
      </form>
      <div className="text-sm mt-3">Already have an account? <Link className="text-pink-600 underline" to="/login">Login</Link></div>
    </div>
  );
}
